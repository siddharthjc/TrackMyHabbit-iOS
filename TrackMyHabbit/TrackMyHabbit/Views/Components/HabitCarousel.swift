...
struct HabitCarousel: View {
    ...
    private func cardLayer(at index: Int, depth: Int) -> some View {
        ...
        let entry = HabitEntryStore.preferredEntry(in: habit.entries, dateString: dateStr)
    ...
    private func saveImage(_ data: Data, for dateString: String) {
        do {
            try HabitEntryStore.savePhoto(data, habit: habit, dateString: dateString, modelContext: modelContext)
        } catch {
