...
struct HabitCarousel: View {
    ...
    private func cardLayer(at index: Int, depth: Int) -> some View {
        ...
        let entry = habit.entries.first(where: { $0.dateString == dateStr && $0.imageUri != nil })
    ...
    private func saveImage(_ data: Data, for dateString: String) {
        do {
            try HabitEntryStore.savePhoto(data: data, habit: habit, dateString: dateString, modelContext: modelContext)
        } catch {
