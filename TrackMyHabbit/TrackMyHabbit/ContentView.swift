...
struct ContentView: View {
    ...
    private func deleteEntry(for habit: Habit, dateStr: String) {
        do {
            try HabitEntryStore.deleteEntries(for: habit, dateString: dateStr, modelContext: modelContext)
        } catch {
            print("Failed to delete photo entry for \(habit.name): \(error.localizedDescription)")
        }
    }

    private func savePhoto(for habit: Habit, dateStr: String, data: Data) {
        do {
            try HabitEntryStore.savePhoto(
                data: data,
                habit: habit,
                dateString: dateStr,
            ...
            print("Failed to save photo for \(habit.name): \(error.localizedDescription)")
        }
    }
